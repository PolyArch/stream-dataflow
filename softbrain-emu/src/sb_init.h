...
#define FIX_MUL(a, b) ( FIX_TRUNC( ((int)a * (int)b) >> FRAC_BITS ) )
#define FIX_TAN_H(x) ( DOUBLE_TO_FIX(tanh(FIX_TO_DOUBLE(x))) )

extern uint64_t accum;

inline float as_float(std::uint32_t ui) {
  float f;
...
inline uint32_t as_uint32(float f) {
  uint32_t ui;
  std::memcpy(&ui, &f, sizeof(uint32_t));
  return ui;
}

inline double as_double(std::uint64_t ui) {
  double f;
...
inline uint64_t as_uint64(double f) {
  uint64_t ui;
  std::memcpy(&ui, &f, sizeof(uint64_t));
  return ui;
}
#endif
